...
Cplx C0Q(Double p10, Double p12, Double p20, Double m0, Double m1, Double m2)
{
  ...
  if (abs(p10) <= ZERO_LIMIT || abs(p12) <= ZERO_LIMIT || abs(p20) <= ZERO_LIMIT)
  {
    ...
    if (m0 * m1 == Double(0))
    {
      ...
      for (j = 0; j < 2; j++)
        c0 += li2(1. + x[1][j]) - li2(1. + x[1][j] / al) - Eta(1. / al, -x[1][j]) * log(1. + x[1][j] / al);
      c0 += li2(1. + x[0][0] / al) - li2(1. + x[0][0]) + Eta(1. / al, -x[0][0]) * log(1. + x[0][0] / al);
    ...
    else
    {
      ...
      {
        for (j = 0; j < 2; j++)
          c0 += (1 - 2 * i) * (li2(1. + x[i][j] / al) - li2(1. + x[i][j]) + Eta(1. / al, -x[i][j]) * log(1. + x[i][j] / al));
      }
      c0 += log(al) * log(m0 / m1);
      c0 /= (p20 - p12);
      return c0;
  ...
  if (abs(al) <= ZEROS_LIMIT)
  {
    ...
    {
      for (j = 0; j < 2; j++)
        c0 += ((1 - x[i][j]) * log((1 - x[i][j]) / y0[i]) + x[i][j] * log(-x[i][j] / y0[i])
               /*      - Eta(1.-x[i][j], 1/y0[i]) * (1.-x[i][j])
	        - Eta(-x[i][j], 1/y0[i]) * x[i][j]*/
               ) /
              y0[i];
  ...
  y0[0] = (p12 * (p12 - p10 - p20 + 2 * m0 - m1 - m2) - (p20 - p10) * (m1 - m2) + al * (p12 - m1 + m2)) / (2 * al * p12);
  y0[1] = (p20 * (p20 - p12 - p10 + 2 * m1 - m2 - m0) - (p10 - p12) * (m2 - m0) + al * (p20 - m2 + m0)) / (2 * al * p20);
  y0[2] = (p10 * (p10 - p20 - p12 + 2 * m2 - m0 - m1) - (p12 - p20) * (m0 - m1) + al * (p10 - m0 + m1)) / (2 * al * p10);
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    {
      c0 -= log((y0[i] - 1.) / y0[i]) * (Eta(-x[i][0], -x[i][1]) - Eta(y[i][0], y[i][1]) - TwoPiI * thp[i] * thy[i]);
    }
  }
...
Cplx DM3C0(Double p10, Double p20, Double p12, Double m1, Double m2, Double m3)
{
  ...
  return (((8 * m1 * (-(powint(m1, 3) * p20 * (m2 - m3 + p20)) + p10 * (-(sqr(m2 - m3) * (sqr(m1 - m3) - (m1 + m3) * p12)) + (m2 + m3) * (-m1 + m3 - p12) * (-m1 + m3 + p12) * p20 - (m1 + m3 - p12) * p12 * sqr(p20)) + sqr(m1) * (m2 * (m3 - p20) * (-2 * p12 + p20) + sqr(m2) * (p12 + p20) + (m3 - p20) * (m3 * (p12 - 2 * p20) - 3 * p12 * p20)) + m2 * (m3 - p12) * (sqr(m2) * p12 + (p12 - p20) * (sqr(m3) + m3 * p20 - 2 * p12 * p20) + m2 * (-3 * p12 * p20 + m3 * (-2 * p12 + p20))) - m1 * (powint(m2, 3) * p12 - sqr(m2) * (m3 - p12) * (p12 - 2 * p20) + (m3 - p20) * (p12 - p20) * (m3 * (m3 + p12) - 2 * p12 * p20) - m2 * (sqr(m3) * (p12 + p20) - 3 * p12 * p20 * (p12 + p20) + 2 * m3 * (sqr(p12) + sqr(p20)))))) / (den1 * den2) +
           (4 * sqr(m1) * (m1 * (p20 + p12 - p10) + p12 * (-2 * m2 + p20 - p12 + p10) + m3 * (-p20 + p12 + p10)) * B0A(0, m1, m1)) / den1 +
           (4 * m1 * m2 * (-2 * m1 * p20 + m3 * p20 - sqr(p20) - m3 * p12 + p20 * p12 + m2 * (p20 + p12 - p10) + m3 * p10 + p20 * p10) * B0A(0, m2, m2)) /
               den2 +
           (((-m2 + m3 + p20) * (m1 + m3 - p12) * (sqr(m1) + sqr(m2 - p10) - 2 * m1 * (m2 + p10))) / den2 +
            ((m1 * m1 + (m2 - p10) * (m3 - p12) - m1 * (m2 + m3 + p10 + p12 - 2 * p20)) *
             (2 * m1 * (-m1 + m3 + p12) / den1 + (m1 + m2 - p10) * (m2 - m3 - p20) / den2))) *
               B0A(0, m3, m3) +
           (4 * m1 * p20 * (-sqr(m2) + m1 * (m2 - m3 + p20) + (m3 - p20) * p10 + m2 * (m3 + p20 - 2 * p12 + p10)) * B0A(p20, m2, m3)) / den2 -
           (4 * m1 * p12 * (sqr(m1) + (m3 - p12) * (m2 - p10) - m1 * (m2 + m3 - 2 * p20 + p12 + p10)) * B0A(p12, m1, m3)) / den1 +
           4 * m1 * p10 * B0A(p10, m1, m2)) /
          (4. * m1 * (sqr(m1) * p20 + m2 * (m3c - p12) * (p20 - p12) + sqr(m2) * p12 - m2 * (m3c + p12) * p10 + p10 * ((m3c - p20) * (m3c - p12) + m3c * p10) - m1 * ((m3c - p20) * (p20 - p12) + m2 * (p20 + p12 - p10) + (m3c + p20) * p10))));
}

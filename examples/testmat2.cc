// Demonstrate use of exact-NLO and linearlized NNLO cross-section calculation
...
// `standard' method, passing the FA and SW objects to the matrix elements:
...
// faster method, passing the FA and SW output to the matrix elements:
// (but in this case the values of FA and SW will not be automatically updated
// when the SM input parameters change)
  matel M0(ini, fin, iff, off, FAi0.result().real(), FAf0.result().real(), SWi0.result().real(), SWf0.result().real(), cme*cme, cost, myinput);
  mat_SMeNLO M1(ini, fin, iff, off, FAi1.result().real(), FAf1.result().real(), SWi1.result().real(), SWf1.result().real(), cme*cme, cost, myinput);
  mat_SMNNLO M2(ini, fin, iff, off, FAi2.result().real(), FAf2.result().real(), SWi2.result().real(), SWf2.result().real(), cme*cme, cost, myinput);

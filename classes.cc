...
Cplx matel::coeffR(void) const
{
  ...
  {
  case VEC:
    zi = 2 * I3f[it] * sqrt(FAi->result()) * (1. - 4 * fabs(Qf[it]) * realreg(SWi->result()));
    break;
  case AXV:
    zi = 2 * I3f[it] * sqrt(FAi->result());
    break;
  }
  switch (off)
  {
  case VEC:
    zf = 2 * I3f[ot] * sqrt(FAo->result()) * (1. - 4 * fabs(Qf[ot]) * realreg(SWo->result()));
    break;
  case AXV:
    zf = 2 * I3f[ot] * sqrt(FAo->result());
    break;
  }
  return (zi * zf);
}
...
// matrix element neither using laurent expansion nor dyson resummation
Cplx matel::mateloffp1(void) const
{
  double mz = ival->get(MZ),
         gz = ival->get(GamZ),
         gie0 = g0(it, iff, *ival),
  ...
  double
      sminusmzs(s - mz * mz),
      matelexp1(zie0 * zjf0 / sminusmzs + gie0 * gjf0 / s);

  return (matelexp1);
}

// laurent expanding the matrix element with gamz = 0
Cplx matel ::mateloffp2(void) const
{
  double mz = ival->get(MZ),
         gz = 0,
         gie0 = g0(it, iff, *ival),
         ...
         zie0 = z0(it, iff, *ival),
         zjf0 = z0(ot, off, *ival);
  Cplx matelexp2(zie0 * zjf0 / (s - mz * mz) + vg0(it, *ival) * vg0(ot, *ival) / (mz * mz) - vg0(it, *ival) * vg0(ot, *ival) / (mz * mz * mz * mz) * (s - mz * mz));
  return (matelexp2);
}

Cplx matel::result2(void) const
{
  return (mateloffp1() - mateloffp2());
}
...
Cplx matelsq::result(void) const
{
  ...
  return ((FAi->result() * FAo->result() * ((if1 == VEC) ? (1. - 4 * fabs(Qf[it]) * realreg(SWi->result())) : 1) * ((if2 == VEC) ? (1. - 4 * fabs(Qf[it]) * realreg(SWi->result())) : 1) * ((of1 == VEC) ? (1. - 4 * fabs(Qf[ot]) * realreg(SWo->result())) : 1) * ((of2 == VEC) ? (1. - 4 * fabs(Qf[ot]) * realreg(SWo->result())) : 1) + gz * gz / (mz * mz) * gie0 * gjf0 * gke0 * glf0 + (s / (mz * mz) - 1) * Yijkl + sqr(s / (mz * mz) - 1) * Vijkl) / (sqr(s - mz * mz) + sqr(mz * gz)));
}
